#!/usr/bin/env bash
# run_synthetic.sh — Train all model variants on synthetic-merchant and print results.
#
# Usage:
#   bash run_synthetic.sh [--device cuda|mps|cpu] [--epoch N] [--seed N]
#
...
echo "---- selfGNN-Base done. Result: $RESULTS_DIR/synthetic_merchant_base.json"
...
echo "---- selfGNN-Feature (edge) done. Result: $RESULTS_DIR/synthetic_merchant_edge.json"
...
(
  ...
  python train.py \
    ...
    --use_edge_features \
    --use_node_features \
    --save_path "synthetic_merchant_full"
)
echo "---- selfGNN-Feature (full) done. Result: $RESULTS_DIR/synthetic_merchant_full.json"
...
for TAG in synthetic_merchant_base synthetic_merchant_edge synthetic_merchant_full; do
